...
default
{
    ...
    touch_start(integer duh)
    {
        ...
        if(PrimaryName != "")
        {
            ...
            if(InUse == NULL_KEY || InUse == llDetectedKey(0))
            {
                ...
                llMessageLinked(LINK_SET,53,PrimaryName+"--"+SecondaryName,"UPDATEITEMS");
                //Inform all other Primary's we've been changed
                llMessageLinked(LINK_SET,250,llGetObjectName(),"BUTTONSELECTED");
            }
    ...
    link_message(integer link, integer num, string msg, key k)
    {
        ...
        //Is it OUR packet?
        else if(num == (250+(integer)llGetObjectName()))
        {
            ...
            if(k == "SETBUTTON")
            {
                ...
                ButtonPic   = llList2Key(lDecode,2);
                //Now set our texture
